...
/**
 * ✅ PERFORMANCE REVISION (iOS Safari freeze fix) — TASARIM AYNI
 * - Devasa blur bloblar kaldırıldı -> aynı görünüm için radial-gradient overlay (blur filtresi yok)
 * - Form state + typing re-render izolasyonu: Form ayrı memo component -> tüm sayfa her tuşta render olmaz
 * - Left panel kendi state'ine sahip (map load vs.) -> form yazarken soldaki kısım hiç etkilenmez
 */

type MotionInFn = (delay?: number) => any
...
const ContactForm = memo(function ContactForm({
  ...
  const [isSubmitted] = useState(false) // tasarım/akış aynı kalsın; mevcut kod redirect yaptığı için burada da aynı
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
      ...
      // ✅ iOS Safari için: input typing sırasında ekstra frame işleri yapma; direkt set et.
      // (Parent artık render olmayacağı için bu yeterli.)
      if (isMobile) {
        setFormData((prev) => ({ ...prev, [name]: value }))
        return
      }

      // Desktop/diğer: yine de yumuşak update
  ...
  const handleSubmit = useCallback(
    ...
    [consents, formData, router, setIsLoading]
  )
...
export default function IletisimPage() {
  ...
  // Client hazır olmadan render etme
  if (!isClient) {
    return (
    ...
    <LazyMotion features={domAnimation}>
      <>
        <div className="min-h-screen pt-28 pb-12 sm:pt-36 sm:pb-20 relative overflow-hidden">
          {/* ✅ SAME LOOK, CHEAPER BACKGROUND (no blur filter) */}
          <div className="absolute inset-0 pointer-events-none">
            <div
              className="absolute inset-0"
              style={{
                background:
                  'radial-gradient(600px 600px at 25% 0%, rgba(56,189,248,0.18), transparent 60%),' +
                  'radial-gradient(520px 520px at 75% 100%, rgba(168,85,247,0.16), transparent 60%)',
                opacity: 0.28,
              }}
            />
          </div>
          ...
          <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid lg:grid-cols-12 gap-6 lg:gap-8">
              {/* Left (memo) */}
              <LeftPanel motionIn={motionIn} features={features} />

              {/* Right (memo + isolated state) */}
              <ContactForm motionIn={motionIn} isMobile={isMobile} reduce={!!reduce} />
            </div>
          </div>
        </div>
